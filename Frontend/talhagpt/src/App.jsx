...
/>
            ...
            <select className="select select-sm bg-gray-800/80 text-white w-28" value={selectedCharacter} onChange={(e) => { setSelectedCharacter(e.target.value); setChatHistory([]); }}>
              <option disabled value="">Character</option>
              {characters.map((c) => (
                <option key={c}>{c}</option>
              ))}
            </select>
            <select className="select select-sm bg-gray-800/80 text-white w-24" value={selectedRole} onChange={(e) => setSelectedRole(e.target.value)}>
              <option disabled value="">Role</option>
              {roles.map((r) => (
                <option key={r}>{r}</option>
              ))}
            </select>
            <input type="number" step="0.1" min="0.1" max="1" className="input input-sm w-20 bg-gray-800/80 text-white" value={temperature} onChange={(e) => setTemperature(parseFloat(e.target.value))} placeholder="Temp" title="Set temperature (0.1–1.0)" />
        ...
        <div className="modal">
          <div className="modal-box w-11/12 max-w-4xl bg-gray-900 text-white overflow-y-auto max-h-[90vh]">
            <div className="flex justify-between items-center mb-2">
              ...
              <label htmlFor="chainModal" className="btn btn-sm btn-circle btn-error">✕</label>
            </div>
            <div className="flex flex-wrap gap-4">
              {Object.entries(chains).map(([chainName, chainData]) => {
                ...
                return (
                  <div key={chainName} className="bg-gray-800 rounded-xl p-4 w-full sm:w-[45%] text-xs shadow-md">
                    <div className="flex justify-between items-center mb-2">
                      <input className="bg-transparent font-semibold truncate outline-none text-white text-sm" value={chainName} onChange={(e) => {
                    ...
                    <input type="number" step="0.1" min="0.1" max="1" value={temp} onChange={(e) => setChains({ ...chains, [chainName]: { ...chainData, temperature: parseFloat(e.target.value) } })} className="input input-xs mt-1 bg-gray-800 text-white w-24" />
                    <div className="flex flex-wrap gap-1 mt-2">
                      {items.map((item, idx) => (
                        <div key={idx} className="bg-gray-700 rounded-full px-2 py-1 flex items-center gap-1 hover:bg-gray-600">
                          <select className="select select-xs bg-gray-800/80 text-xs text-white border-none" value={item.role} onChange={(e) => updateChainItem(chainName, idx, "role", e.target.value)}>
                            <option disabled value="">Role</option>
                            {roles.map((r) => (
                              <option key={r} value={r}>{r}</option>
                            ))}
                          </select>
                          <input className="bg-transparent border-none outline-none w-24 text-xs" value={item.prompt} onChange={(e) => updateChainItem(chainName, idx, "prompt", e.target.value)} placeholder="Prompt" />
                          <button className="text-red-300 hover:text-red-500" onClick={() => removeChainItem(chainName, idx)}>×</button>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="mt-4">
              <button onClick={addNewChain} className="btn btn-sm btn-accent">➕ Add Chain</button>
            </div>
          </div>
        </div>
      </div>
